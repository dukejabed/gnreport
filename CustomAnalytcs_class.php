...
class CustomAnalytcs_class {
    ...
    public function getRealtimeData() {
        ...
        $data = $analytics->data_realtime->get(
                'ga:' . $profileId, 'rt:activeUsers');
        $rowdata = $data->getRows();
        $activeuser = $data->getTotalsForAllResults();
        $realtimedata = array(
            'rows' => $rowdata['0'],
            'totalRows' => $data->getTotalResults(),
            'activeusers' => $activeuser['rt:activeUsers'],
            'columnHeader' => $data->getColumnHeaders()
        );
        return $realtimedata;
    }

    public function getSessonData() {
        $analytics = $this->getService();
        $profileId = $this->getFirstProfileId($analytics);
        $optParams = array(
        ...
        $data = $analytics->data_ga->get(
                'ga:' . $profileId, date('j').'daysAgo', 'today', 'ga:sessions,ga:users,ga:newUsers,ga:bounceRate,ga:sessionDuration,ga:avgSessionDuration,ga:hits,ga:visits');
        $rowdata = $data->getRows();

        $data1 = $analytics->data_ga->get(
                'ga:' . $profileId, '30daysAgo', 'today', 'ga:sessions,ga:users,ga:newUsers,ga:bounceRate,ga:sessionDuration,ga:avgSessionDuration,ga:hits,ga:visits', $optParams);
